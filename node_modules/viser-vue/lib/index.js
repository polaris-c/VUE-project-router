...
var regSeries = ['pie', 'sector', 'line', 'smoothline', 'dashline', 'area', 'point', 'stackarea', 'smootharea', 'bar', 'stackbar', 'dodgebar', 'interval', 'stackinterval', 'dodgeinterval', 'funnel', 'pyramid', 'schema', 'box', 'candle', 'polygon', 'contour', 'heatmap', 'edge', 'sankey', 'errorbar'];
var rootCharts = ['v-chart', 'v-lite-chart'];
var rootChartProps = ['data', 'dataView', 'dataPre', 'scale', 'viewId'];
...
var baseChartComponent = {
    ...
    methods: {
        findNearestRootComponent: function findNearestRootComponent(componentInstance) {
            if (componentInstance.isViser && rootCharts.concat(['v-view', 'v-facet', 'v-facet-view']).indexOf(componentInstance.$options._componentTag) > -1) {
        ...
        freshChart: function freshChart(isUpdate) {
            if (rootCharts.indexOf(this.$options._componentTag) > -1) {
                var d2Json_1 = __assign({}, cleanUndefined(normalizeProps(this._props, rootChartProps)), { chart: __assign({ container: this.$el }, cleanUndefined(normalizeProps(this._props, null, rootChartProps))) }, this.jsonForD2);
                if (this.$options._componentTag === 'v-lite-chart') {
                    ...
                    Object.keys(existProps_1).forEach(function (propsKey) {
                        ...
                        if (regSeries.indexOf(lowerCasePropsKey) > -1) {
                            safePush(d2Json_1, 'series', __assign({ quickType: propsKey }, normalizeProps(existProps_1, seriesProps)));
                        }
                    });
                    setIfNotExist(d2Json_1, 'axis', true);
                    setIfNotExist(d2Json_1, 'legend', true);
                    setIfNotExist(d2Json_1, 'tooltip', true);
                }
                if (!isUpdate || !this.chart) {
                    this.chart = viser["default"](d2Json_1);
                } else {
                    this.chart.repaint(d2Json_1);
                }
            } else if (this.$options._componentTag === 'v-view') {
                var nearestRootComponent = this.findNearestRootComponent(this.$parent);
                oneObjectMoreArray(nearestRootComponent.jsonForD2, 'views', __assign({}, cleanUndefined(normalizeProps(this._props)), this.jsonForD2, { viewId: this._props.viewId || generateRandomNum() }));
